...
import Hedgehog.Internal.Property as Property (ShrinkPath (ShrinkPath))
...
prop_ShrinkNoLimit =
  withTests 1 . property $ do
    ...
    case reportStatus report of
      Failed f -> do
        5 === failureShrinks f
        ShrinkPath [1, 0, 0, 1, 0] === failureShrinkPath f
      _ -> failure
...
prop_ShrinkLimit =
  withTests 1 . property $ do
    ...
    case reportStatus report of
      Failed f -> do
        3 === failureShrinks f
        ShrinkPath [1, 0, 0] === failureShrinkPath f
      _ -> failure
...
prop_ShrinkTimeoutMicrosZero =
  withTests 1 . property $ do
    ...
    case reportStatus report of
      Failed f -> do
        0 === failureShrinks f
        ShrinkPath [] === failureShrinkPath f
      _ -> failure
...
prop_ShrinkTimeoutMicros =
  let delay x = when (x == 13) (liftIO $ CC.threadDelay delayTime)
    ...
    case reportStatus report of
      Failed f -> do
        ...
        case failureAnnotations f of
          [ann] -> do
            "25" === failedValue ann
            ShrinkPath [1] === failureShrinkPath f
          _ -> failure
      _ -> failure
...
prop_ShrinkTimeoutMicrosClock =
  let delay30s x = when (x == 13) (liftIO $ CC.threadDelay 30000000)
  in withTests 1 . property $ do
    ...
    -- Upper bound relaxed to 15 seconds to prevent test failure from e.g.
    -- CPU starvation / GC. While this is pretty lax, it is far from the
    -- non-limited expected time (30 seconds), so it still demonstrates
    -- that the timeout eventually fired.
    diff timeElapsed (<=) 15
#else
